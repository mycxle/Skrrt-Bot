...
from globals import *
import discord
from custom_role_creator import CustomRoleCreator
...
class ShopCommands:
    ...
    async def buy(self, ctx, *args):
        ...
        if category == "role" or category == "roles":
            if option == "custom":
                if ctx.message.author.id not in Global.role_creators:
                    crc = CustomRoleCreator()
                    Global.role_creators[str(ctx.message.author.id)]=[ctx.message.channel.id, crc]
                    await self.bot.say(ctx.message.author.mention + " " + crc.get_current())
            else:
                try:
